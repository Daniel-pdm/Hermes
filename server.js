  ...
  try {
    ...
    while (run < MAX_PAGES_PER_RUN) {
      ...
      for (const a of arts) {
        ...
        let body = "";
        try { const full = await fetchArticle(String(a.id)); body = full.text || ""; } catch { /* skip body */ }
        if (!body) continue;
        // Store as one fact: title + trimmed body. Trim very long articles so a
        // single row stays reasonable for keyword retrieval.
        const factText = `[Article] ${title}: ${body.slice(0, 1500)}`;
        await storeFact(factText, { sourceType: "article", sourceRef: url, confidence: 0.75 });
        processed++;
      }
      ...
      console.log(`article ingest: page ${page - 1}/${totalPages} done (${processed} stored this run).`);
      if (page > totalPages) {
...
app.post("/slack/events", (req, res) => {
  ...
  if (LEARN_CHANNEL_SET.has(channel)) {
    (async () => {
      ...
      // Keep the newest-ts marker current so the daily incremental refresh knows
      // where it left off.
