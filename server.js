...
async function ingestAllArticles() {
  ...
  try {
    ...
    while (run < MAX_PAGES_PER_RUN && nextUrl) {
      ...
      if (arts.length === 0 && (data.total_count === 0 || data.total_count === undefined)) {
        console.error("article ingest: API returned NO articles — likely the INTERCOM_TOKEN lacks Help Center 'read articles' (list) permission, or points at the wrong workspace. Individual article reads can still work while listing is blocked.");
      }
      let firstDiag = true;
      for (const a of arts) {
