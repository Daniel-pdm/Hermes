...
async function extractAndLearn(userText, meta = {}) {
  ...
  try {
    const sys =
      ...
      "sentence ONLY IF the message asserts something durable and reusable, such as: a feature " +
      "change, a removed/renamed/added feature, a known bug, a workaround, a product clarification, " +
      "product terminology, or a resolved issue.\n\n" +
      "Return the exact text NONE (nothing else) if the message is: a question, a greeting, small " +
      "talk, an opinion, a request for help, or anything not stated as a durable product fact. " +
      "When unsure, return NONE — it is much better to skip than to store noise. Never infer or " +
      "invent beyond what the message plainly states. Do not include names, pleasantries, or " +
      "commentary — just the fact as a clean standalone sentence.";
    ...
    while (pages < MAX_PAGES_PER_RUN) {
      ...
      for (const m of (j.messages || [])) {
        ...
        if (t.length >= 15) {
          ...
          // Reuse the same conservative extractor; tag source as the channel.
          await extractAndLearn(t, { author: m.user || null, sourceType: "slack", sourceRef: FEEDBACK_CHANNEL_ID });
          // extractAndLearn logs its own result; we can't easily know stored vs not here,
          // so we just count scanned. (Dedup + NONE-filtering happen inside.)
          if (learned === before) { /* no-op */ }
        }
      }
...
app.post("/slack/events", (req, res) => {
  ...
  if (channel === FEEDBACK_CHANNEL_ID) {
    if (text) {
      extractAndLearn(text, { author: ev.user || null, sourceType: "slack", sourceRef: FEEDBACK_CHANNEL_ID });
    }
    return;
  }
