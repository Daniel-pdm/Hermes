    ...
    while (pages < MAX_PAGES_PER_RUN) {
      ...
      for (const m of (j.messages || [])) {
        ...
        if (hasThread) {
          ...
          if (turns.length) {
            await learnFromThread(turns, { author: m.user || null, sourceType: "slack", sourceRef: FEEDBACK_CHANNEL_ID });
            await sleep(250);                              // gentle pacing on Slack API
            continue;
          }
        }
        if (t.length >= 15) {
          await extractAndLearn(t, { author: m.user || null, sourceType: "slack", sourceRef: FEEDBACK_CHANNEL_ID });
        }
      }
  ...
  if (channel === FEEDBACK_CHANNEL_ID) {
    (async () => {
      ...
      if (ev.thread_ts) {
        ...
        if (turns.length) { await learnFromThread(turns, { author: ev.user || null, sourceType: "slack", sourceRef: FEEDBACK_CHANNEL_ID }); return; }
      }
      if (text) await extractAndLearn(text, { author: ev.user || null, sourceType: "slack", sourceRef: FEEDBACK_CHANNEL_ID });
    })();
    return;
  }
