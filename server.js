...
// Slack channel Hermes LEARNS from (reads, never replies). Set in Render as
// FEEDBACK_CHANNEL_ID; defaults to #creative-affinity-feedback.
const FEEDBACK_CHANNEL_ID = process.env.FEEDBACK_CHANNEL_ID || "C0ANW7CFQUT";

// Build a precise Slack permalink to a thread from its channel + timestamp.
// e.g. ts "1786453912.423659" -> https://<ws>/archives/<CH>/p1786453912423659
    ...
    if (facts.length) {
      learnedBlock =
        ...
        facts.map(f => {
          ...
          return `• (${f.source_type}, confidence ${Number(f.confidence).toFixed(2)})${link ? ` [thread: ${link}]` : ""} ${f.fact}`;
        }).join("\n") +
...
async function runBackfill() {
  ...
  const doneFlag = await getMeta("backfill_done");
  if (doneFlag === "1") { console.log("backfill: already complete."); return; }

  BACKFILL_RUNNING = true;
  console.log(`backfill: starting/resuming for channel ${FEEDBACK_CHANNEL_ID} …`);
  try {
    let cursor = (await getMeta("backfill_cursor")) || "";
    let pages = 0, learned = 0, scanned = 0;
    // Process a bounded number of pages per run so a single invocation stays
    // well under the free-tier idle window; startup will resume the rest.
    const MAX_PAGES_PER_RUN = 8;      // ~8 x 100 msgs = 800 msgs per run
    while (pages < MAX_PAGES_PER_RUN) {
      const params = new URLSearchParams({ channel: FEEDBACK_CHANNEL_ID, limit: "100" });
      if (cursor) params.set("cursor", cursor);
      const res = await fetch(`https://slack.com/api/conversations.history?${params}`, {
        headers: { "Authorization": `Bearer ${SLACK_BOT_TOKEN}` }
      });
      const j = await res.json();
      if (!j.ok) { console.error("backfill: conversations.history failed:", j.error); break; }

      for (const m of (j.messages || [])) {
        if (m.subtype || m.bot_id) continue;              // skip system/bot msgs
        const t = (m.text || "").replace(/<@[^>]+>/g, "").trim();
        scanned++;
        // A message with replies is a thread — pull the whole exchange so we
        // capture the SOLUTION, not just the report. conversations.history marks
        // thread parents with reply_count > 0.
        const hasThread = (m.reply_count && m.reply_count > 0);
        if (hasThread) {
          const turns = await fetchThreadTurns(FEEDBACK_CHANNEL_ID, m.ts);
          if (turns.length) {
            await learnFromThread(turns, { author: m.user || null, sourceType: "slack", channelId: FEEDBACK_CHANNEL_ID, threadTs: m.ts });
            await sleep(250);                              // gentle pacing on Slack API
            continue;
          }
        }
        if (t.length >= 15) {
          await extractAndLearn(t, { author: m.user || null, sourceType: "slack", channelId: FEEDBACK_CHANNEL_ID, threadTs: m.ts });
        }
      }

      pages++;
      cursor = (j.response_metadata && j.response_metadata.next_cursor) || "";
      await setMeta("backfill_cursor", cursor);
      console.log(`backfill: page ${pages} done, scanned ~${scanned} msgs so far, cursor ${cursor ? "saved" : "END"}.`);

      if (!cursor) {                                       // no more pages -> complete
        await setMeta("backfill_done", "1");
        console.log(`backfill: COMPLETE. Scanned ~${scanned} messages total this run.`);
        break;
      }
    }
    if (cursor) {
      console.log("backfill: batch limit reached; will resume on next restart/deploy.");
    }
  } catch (e) {
...
app.post("/slack/events", (req, res) => {
  ...
  // If the message is in the feedback channel, LEARN from it silently — never
  // reply, never react. Hermes is a listener there, not a chat bot.
  if (channel === FEEDBACK_CHANNEL_ID) {
    (async () => {
      // If this message is part of a thread, learn from the whole exchange
      // (problem + solution). Otherwise learn from the single message.
      if (ev.thread_ts) {
        const turns = await fetchThreadTurns(FEEDBACK_CHANNEL_ID, ev.thread_ts);
        if (turns.length) { await learnFromThread(turns, { author: ev.user || null, sourceType: "slack", channelId: FEEDBACK_CHANNEL_ID, threadTs: ev.thread_ts }); return; }
      }
      if (text) await extractAndLearn(text, { author: ev.user || null, sourceType: "slack", channelId: FEEDBACK_CHANNEL_ID, threadTs: ev.ts });
    })();
    return;
  }
  ...
  if (attack) {
    ...
    (async () => {
      ...
      await postSlack(channel, thread_ts,
        "I can only help with nova Intelligence and Creative Affinity questions, and I can't share " +
        "system details or credentials. Happy to help with a Creative Affinity question though!");
...
app.listen(PORT, () => {
  ...
  // Connect the store, then kick off the resumable channel backfill once it's ready.
  initStore().then((ready) => {
    if (ready) {
      // Small delay so startup logs settle; backfill resumes from its saved cursor.
