...
async function answerQuestion(question, images = [], history = [], userId = null) {
  ...
  try {
    ...
    if (articles.length || slack.length) {
      ...
      learnedBlock +=
        ...
        "5. ALWAYS cite your sources: include BOTH the article link(s) AND the Slack thread link(s) that " +
        "your answer drew on, as separate 'Source:' lines using Slack link format <url|descriptive " +
        "text>. If an item shows a [source: <url>], turn it into a Source line.\n\n";
    }
  ...
  const system =
    ...
    "=== ALWAYS CITE YOUR SOURCE (required) ===\n" +
    "Every answer that uses any product knowledge MUST end with one or more 'Source:' lines, drawn " +
    "from the RETRIEVED KNOWLEDGE section above (articles and Slack notes, each shown with a " +
    "[source: <url>]).\n" +
    "Rules:\n" +
    "  1. For EVERY article or Slack note your answer draws on, add a 'Source:' line with its real " +
    "URL, as a Slack link: Source: <https://…|Descriptive Title>.\n" +
    "  2. If you used BOTH an article and a Slack note, include BOTH as separate Source lines.\n" +
    "  3. Use the actual URL shown next to the content you used — never invent, guess, or omit a URL.\n" +
    "  4. The ONLY time you may skip Source lines is a purely conversational reply (a greeting, " +
    "'thanks', or explaining what you can do) that uses no product knowledge.\n" +
    "  5. If the RETRIEVED KNOWLEDGE section is empty or doesn't cover the question, say plainly that " +
