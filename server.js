...
async function ingestAllArticles() {
  ...
  try {
    ...
    while (run < MAX_PAGES_PER_RUN) {
      ...
      console.log(`article ingest: page ${page} returned ${arts.length} article(s); total_pages=${data.pages && data.pages.total_pages}`);
      let firstDiag = true;
      for (const a of arts) {
        ...
        try {
          ...
          body = full.text || "";
          if (firstDiag) { console.log(`article ingest: sample fetch ${a.id} -> body length ${body.length}`); firstDiag = false; }
        } catch (e) {
