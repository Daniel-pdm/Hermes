...
const LEARN_CHANNELS = (process.env.LEARN_CHANNELS ||
  // original 3 (fully backfilled) + 4 new (180-day limited)
...
// Backfill history limit: only ingest messages newer than this many days.
// The original 3 channels were already fully backfilled (their done-flags are
// set, so they won't re-scan). New channels honor this cutoff. 0 = no limit.
...
async function runBackfill() {
  ...
  try {
    for (const channelId of LEARN_CHANNELS) {
      ...
      // 180-day (configurable) cutoff: ask Slack only for messages newer than
      // this. Slack's `oldest` is a unix-seconds timestamp; older messages are
      // simply not returned, so the backfill naturally stops at the cutoff.
      const oldestTs = BACKFILL_MAX_AGE_DAYS > 0
