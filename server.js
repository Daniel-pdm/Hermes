...
async function answerQuestion(question, images = [], history = []) {
  ...
  try {
    ...
    if (articles.length || slack.length) {
      ...
      learnedBlock +=
        "\nHOW TO USE BOTH:\n" +
        "1. Draw on BOTH the article knowledge and the Slack knowledge above — do not rely on only one.\n" +
        "2. If they CONFLICT (a Slack note says something changed/was removed/renamed vs. what an " +
        "article says), LEAD WITH THE SLACK NOTE as the current truth, and add a brief line noting " +
        "the article may be outdated. Slack team knowledge is generally newer than the docs.\n" +
        "3. Among Slack notes, prefer the one with the more recent 'learned' date.\n" +
        "4. ALWAYS cite your sources: include BOTH the article link AND the Slack thread link when " +
        "your answer draws on each, as separate 'Source:' lines using Slack link format " +
        "<url|descriptive text>. If an item shows a [source: <url>], turn it into a Source line.\n\n";
    }
