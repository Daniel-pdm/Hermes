...
import { initStore, storeEnabled, storeFact, retrieveRelevant, retrieveBlended, getMeta, setMeta } from "./store.js";
...
async function answerQuestion(question, images = [], history = [], userId = null) {
  ...
  let learnedBlock = "";
  try {
    const { articles, slack } = await retrieveBlended(question, 4, 4);
    const fmt = (f) => {
