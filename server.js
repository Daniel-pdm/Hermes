...
async function answerQuestion(question, images = [], history = [], userId = null) {
  ...
  try {
    ...
    const kw = await retrieveBlended(searchQuery, 4, 4);
      ...
      try {
        ...
        if (qvec) {
          ...
          const merge = (a, b) => {
            ...
            return a.concat(b.filter(r => !seen.has(r.fact))).slice(0, 6);
          };
          articles = merge(articles, semArticles);
          slack = merge(slack, semSlack);
