...
| `BACKFILL_MAX_AGE_DAYS`  | Only backfill channel history newer than this many days (default 180; 0 = no limit). Applies to channels not yet backfilled; already-done channels keep their full history. |
...
- **Channel backfill** (one-time per channel): reads history in resumable batches.
  New channels honor `BACKFILL_MAX_AGE_DAYS`; already-done channels are skipped.
