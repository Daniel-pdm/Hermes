...
- **Three knowledge sources, blended on every answer:**
  ...
  3. **Intercom support conversations** — the last 12 months of conversations (all states),
     each distilled into its question + resolution, tagged with the product/app it's about.
- **Hybrid retrieval** — semantic (meaning-based) search + keyword (OR-term) search, merged.
  Semantic falls back to keyword automatically if the embedding model can't load. Short
  acronyms (e.g. "BP", "CAC") are matched as whole-word substrings so 2-letter terms aren't
  dropped by full-text search.
- **Cites sources (grouped)** — every knowledge answer ends with a bold **📚 Sources:** block,
  grouped and labeled by type (Article / Slack thread / Intercom conversation), each hyperlinked.
  When an answer cites an Intercom conversation, it appends a note that non-support-team members
  may not have access to those links.
...
- **Learns continuously:**
  - Automatically extracts durable facts from new messages in the feedback channels,
    including feature announcements ("you can now…", launches) — not just bug reports.
  ...
  - **Confirmed vs. reported:** each learned fact is tagged `confirmed` (from an expert,
    a resolution, an announced capability, or a manual `!remember`) or `reported`
    (someone raised/asked about it, unverified). Answers frame `reported` facts as
    "recently raised in Slack," never as established fact / a "known bug."
...
- **Won't hallucinate definitions:** if asked what a term/acronym means (or any claim) and
  the retrieved knowledge doesn't support it, it says it doesn't have that documented and
  asks for clarification — rather than inventing a plausible answer. It grounds claims in
  sources, doesn't fill gaps with invention, and labels any inference as such.
...
embedding + source metadata + `article_id` for articles + a `status` of
`confirmed`/`reported` for Slack/Intercom facts + a `product_tag` for the app a fact
is about). A `kv_meta` table holds
...
| `CONV_MAX_AGE_DAYS`      | How far back to ingest Intercom support conversations (default 365). |
...
**Current learn-channel setup (code defaults):** 9 channels total —
- **Full history:** `C0ANW7CFQUT`, `C0ARYS46EL8`, `C0B81JM0Y9G`, `C0B2WGGBSHW`
- **Last 180 days:** `C01J9CB9GJ2`, `C06LN3NMPQQ`, `C05RCGGN9JS`, `C06DJU1P7CJ`, `C01CJ7V8B1P`

Hermes must be invited to each channel (`/invite @Hermes`) to read it. To change tiers,
edit `LEARN_CHANNELS` and `FULL_HISTORY_CHANNELS` (or set the env vars).
...
- **Conversation ingest** (resumable): pulls Intercom support conversations from the last
  `CONV_MAX_AGE_DAYS` (default 365), all states, via the `/conversations/search` API.
  Each conversation's transcript is distilled by Claude into one fact (question + agent
  resolution), tagged `confirmed`/`reported`, and stored with `source_type = 'intercom'`,
  a link back to the conversation, and the conversation's app/product tag in `product_tag`.
  State: kv_meta `conv_ingest_done`, `conv_ingest_cursor`. NOTE: unlike articles, this
  makes one Claude extraction call per conversation — a real (bounded) one-time cost.
- **Daily refresh:** at most once per 24h (triggered by the first activity that wakes
  the service): new channel messages + article re-check + new Intercom conversations +
  embed new facts.
...
-- Slack facts by confirmed vs reported status
SELECT status, count(*) FROM knowledge WHERE source_type <> 'article' GROUP BY status;

-- Intercom conversation facts by product tag
SELECT product_tag, count(*) FROM knowledge WHERE source_type = 'intercom' GROUP BY product_tag ORDER BY count(*) DESC;

-- How many Intercom conversations have been ingested
SELECT count(*) AS intercom_facts FROM knowledge WHERE source_type = 'intercom';
...
-- Needed to apply extractor changes (e.g. new confirmed/reported tagging or
-- announcement capture) to EXISTING facts; new facts get them automatically.
DELETE FROM kv_meta WHERE k LIKE 'backfill_done:%' OR k LIKE 'backfill_cursor:%';

-- Force a fresh Intercom conversation re-ingest (re-extracts — costs Claude tokens)
DELETE FROM knowledge WHERE source_type='intercom';
DELETE FROM kv_meta WHERE k IN ('conv_ingest_done','conv_ingest_cursor','conv_seen_any');
...
Recommended operational next steps (higher value than more features):
- **Back up the knowledge base.** The Supabase free tier has no backups. Periodically export
  the `knowledge` table (pg_dump or CSV) — it's the most valuable, hardest-to-rebuild asset.
- **Gaps log.** Log every "I don't have that documented" question to a table, so you can see
  exactly what knowledge is missing and fix the source (article/Slack/Intercom) — the most
  effective long-term way to reduce "I don't know" answers and hallucination.
- **Consider the paid Render instance (~$7/mo)** if the team relies on Hermes daily: more RAM
  (semantic search is at ~350MB of 512MB), no ~50s cold starts, and backfills finish in one run
  instead of trickling across wake cycles.
