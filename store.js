...
export async function storeArticle(articleId, factText, sourceRef, confidence = 0.75, sourceType = "article") {
    ...
    try {
      await pool.query(
        `INSERT INTO knowledge (fact, source_type, source_ref, confidence, content_hash, article_id)
         VALUES ($1, $2, $3, $4, $5, $6);`,
        [clean, sourceType, sourceRef || null, confidence, ch, String(articleId)]);
      return "inserted";
    } catch (e) {
...
export async function retrieveBlended(query, nArticles = 4, nSlack = 4) {
  ...
  const [articles, slack] = await Promise.all([
    runScoped(`source_type IN ('article','internal-article')`, nArticles),
    runScoped(`source_type NOT IN ('article','internal-article')`, nSlack)
  ]);
