...
export async function retrieveBlended(query, nArticles = 4, nSlack = 4) {
  ...
  // Articles are source_type 'article'; Slack knowledge is everything else
  // (slack, slack-expert, feedback, manual).
  const [articles, slack] = await Promise.all([
    runScoped(`source_type = 'article'`, nArticles),
    runScoped(`source_type <> 'article'`, nSlack)
  ]);
  console.log(`retrieveBlended "${q.slice(0, 40)}" -> ${articles.length} article(s) + ${slack.length} slack fact(s)`);
