...
async function embed(text, inputType = "document") {
  ...
  let delayMs = 1200;                    // 1.2s → 2.4s → 4.8s backoff on 429
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    // Read the body so we can log WHY it failed (e.g. rate limit vs quota).
    let detail = "";
    try { detail = (await res.text()).slice(0, 200); } catch { /* ignore */ }

    // 429 = rate limit. Honor Retry-After if present, else exponential backoff, then retry.
    if (res.status === 429 && attempt < MAX_ATTEMPTS) {
      const retryAfter = Number(res.headers.get("retry-after"));
      const wait = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1000 : delayMs;
      console.log(`Voyage 429 (rate limit) — retrying in ${Math.round(wait / 1000)}s (attempt ${attempt}/${MAX_ATTEMPTS}). ${detail}`);
      await new Promise(r => setTimeout(r, wait));
      delayMs *= 2;
      continue;
    }

    // Non-429, or out of retries: fail with the detail so logs show the real reason.
