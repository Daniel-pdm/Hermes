...
export async function initStore() {
  ...
  try {
    ...
    try {
      ...
      // Stable per-article identity so article UPDATES replace the row instead
      // of inserting a duplicate. Unique so we can upsert on it.
      await pool.query(`ALTER TABLE knowledge ADD COLUMN IF NOT EXISTS article_id TEXT;`);
      // 'confirmed' (expert/authoritative/resolved) | 'reported' (asked/unconfirmed) |
      // null (articles / manual). Lets answers frame Slack knowledge honestly.
      await pool.query(`ALTER TABLE knowledge ADD COLUMN IF NOT EXISTS status TEXT;`);
      // Product/app tag (e.g. from Intercom conversation tags): 'nova', 'omega',
      // 'signal', 'Integrations', etc. Lets you see/query which product a fact is about.
      await pool.query(`ALTER TABLE knowledge ADD COLUMN IF NOT EXISTS product_tag TEXT;`);
      await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS knowledge_article_id_idx ON knowledge (article_id) WHERE article_id IS NOT NULL;`);
      // One-time migration: existing article rows were stored without an
      // article_id. Backfill it from the numeric ID embedded in their help-center
      // URL (…/articles/<id>-slug), so future updates match them instead of
      // inserting duplicates. Safe to run repeatedly (only fills NULLs).
      await pool.query(`
        UPDATE knowledge
           SET article_id = substring(source_ref from 'articles/([0-9]+)')
         WHERE source_type = 'article'
           AND article_id IS NULL
           AND source_ref ~ 'articles/[0-9]+';
      `);
...
// Remove stored article rows whose article_id is NOT in the given set of
// currently-existing Intercom article IDs (i.e. deleted/unpublished articles).
// SAFETY: only call this after a FULLY COMPLETED ingest. It refuses to run if
// the provided set is suspiciously small vs. what's stored (which would signal
// an incomplete/broken ingest), to avoid mass-deleting the store on a bad pass.
// Returns the number deleted, or -1 if it refused for safety.
export async function reconcileArticles(currentIds) {
  if (!STORE_READY) return 0;
  if (!(currentIds instanceof Set) || currentIds.size === 0) {
    console.error("reconcileArticles: empty/invalid id set — refusing to delete anything.");
    return -1;
  }
  try {
    const { rows } = await pool.query(
      `SELECT count(*)::int AS n FROM knowledge WHERE source_type = 'article' AND article_id IS NOT NULL;`);
    const storedCount = rows[0].n;
    // Guardrail: if we "saw" fewer than half of what's stored, the ingest was
    // almost certainly incomplete — do NOT delete based on it.
    if (storedCount > 0 && currentIds.size < storedCount * 0.5) {
      console.error(`reconcileArticles: saw only ${currentIds.size} ids but ${storedCount} stored — likely incomplete ingest, refusing to delete.`);
      return -1;
    }
    // Delete stored articles not in the current set.
    const ids = [...currentIds].map(String);
    const res = await pool.query(
      `DELETE FROM knowledge
        WHERE source_type = 'article'
          AND article_id IS NOT NULL
          AND NOT (article_id = ANY($1::text[]));`,
      [ids]);
    const deleted = res.rowCount || 0;
    if (deleted) console.log(`reconcileArticles: removed ${deleted} article(s) no longer in Intercom.`);
    else console.log("reconcileArticles: no stale articles to remove.");
    return deleted;
  } catch (e) {
    console.error("reconcileArticles failed:", e.message);
    return -1;
  }
}

// Upserts by article_id so an EDITED article updates the existing row in place
// (new full text replaces old) instead of inserting a duplicate. When the text
// changes, the stored embedding is cleared so the re-embed job regenerates it.
// Returns "inserted" | "updated" | "unchanged" | false.
export async function storeArticle(articleId, factText, sourceRef, confidence = 0.75) {
  if (!STORE_READY) return false;
  const clean = (factText || "").trim();
  if (!articleId || clean.length < 8) return false;
  try {
    const ch = hash(clean.toLowerCase());
    // Is this article already stored, and has its text changed?
    const existing = await pool.query(
      `SELECT id, content_hash FROM knowledge WHERE article_id = $1;`, [String(articleId)]);
    if (existing.rows.length) {
      if (existing.rows[0].content_hash === ch) return "unchanged";   // no edit -> skip
      // Text changed: update in place, and null the embedding so it re-embeds.
      await pool.query(
        `UPDATE knowledge
           SET fact = $1, content_hash = $2, source_ref = $3, confidence = $4,
               embedding = NULL, updated_at = now()
         WHERE article_id = $5;`,
        [clean, ch, sourceRef || null, confidence, String(articleId)]);
      return "updated";
    }
    // New article: plain insert. We already confirmed above (via SELECT) that
    // this article_id doesn't exist, so no ON CONFLICT is needed — which also
    // avoids the partial-unique-index conflict-target limitation.
    try {
      await pool.query(
        `INSERT INTO knowledge (fact, source_type, source_ref, confidence, content_hash, article_id)
         VALUES ($1, 'article', $2, $3, $4, $5);`,
        [clean, sourceRef || null, confidence, ch, String(articleId)]);
      return "inserted";
    } catch (e) {
      // Rare race: another pass inserted this article_id between our SELECT and
      // INSERT. Fall back to updating that row instead of failing.
      if (/duplicate key/i.test(e.message)) {
        await pool.query(
          `UPDATE knowledge SET fact = $1, content_hash = $2, source_ref = $3,
                 confidence = $4, embedding = NULL, updated_at = now()
           WHERE article_id = $5;`,
          [clean, ch, sourceRef || null, confidence, String(articleId)]);
        return "updated";
      }
    ...
    console.error("storeArticle failed:", e.message);
    return false;
  }
}
...
export async function storeFact(fact, meta = {}) {
  ...
  try {
    const ch = hash(clean.toLowerCase());
    await pool.query(
      `INSERT INTO knowledge (fact, source_type, source_ref, author, confidence, content_hash, status, product_tag)
       ...
       ON CONFLICT (content_hash) DO UPDATE SET
         ...
         -- upgrade status to 'confirmed' if a confirmed capture arrives; never downgrade
         status = CASE WHEN EXCLUDED.status = 'confirmed' THEN 'confirmed' ELSE COALESCE(knowledge.status, EXCLUDED.status) END,
         product_tag = COALESCE(knowledge.product_tag, EXCLUDED.product_tag),
         updated_at = now()
       WHERE knowledge.source_ref IS NULL
          ...
          OR (EXCLUDED.status = 'confirmed' AND knowledge.status IS DISTINCT FROM 'confirmed');`,
      [clean, meta.sourceType || "manual", meta.sourceRef || null,
       meta.author || null, meta.confidence ?? 0.5, ch, meta.status || null, meta.productTag || null]
    );
    return true;
  } catch (e) {
...
export async function retrieveSemantic(queryVector, whereClause, limit) {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT fact, source_type, source_ref, confidence, created_at, status,
...
export async function retrieveBlended(query, nArticles = 4, nSlack = 4) {
  ...
  // Normal search terms: length >= 3, not a stopword.
  const terms = words.filter(w => w.length >= 3 && !STOP.has(w));
  // Short terms (2 chars, e.g. "bp") are dropped by full-text search, so we keep
  // them separately and match them as substrings (acronyms like BP, PR, AD).
  const shortTerms = words.filter(w => w.length === 2 && !STOP.has(w));
  // Fallback: if no normal (>=3 char) terms, use the short terms (or a harmless
  // token) so to_tsquery always gets valid OR syntax, never a raw phrase.
  const orQuery = terms.length ? terms.join(" | ")
    : (shortTerms.length ? shortTerms.join(" | ") : "zzzznomatch");

  const runScoped = async (whereClause, limit) => {
    try {
      // Base: full-text OR-match on the meaningful terms.
      // PLUS: if there are short acronym terms, also match rows that contain them
      // as a word (case-insensitive), since full-text search misses 2-char tokens.
      let sql, params;
      if (shortTerms.length) {
        // Word-boundary ILIKE for each short term (e.g. matches "BP" but not "BPM").
        const likeClauses = shortTerms.map((_, i) => `fact ~* ('\\m' || $${i + 3} || '\\M')`).join(" OR ");
        sql =
          `SELECT fact, source_type, source_ref, confidence, created_at, status,
                  GREATEST(
                    ts_rank(fact_tsv, to_tsquery('english', $1)),
                    CASE WHEN (${likeClauses}) THEN 0.5 ELSE 0 END
                  ) AS rank
           FROM knowledge
           WHERE (fact_tsv @@ to_tsquery('english', $1) OR (${likeClauses}))
             AND ${whereClause}
           ORDER BY rank DESC, confidence DESC, created_at DESC
           LIMIT $2;`;
        params = [orQuery, limit, ...shortTerms];
      } else {
        sql =
          `SELECT fact, source_type, source_ref, confidence, created_at, status,
  ...
  console.log(`retrieveBlended "${q.slice(0, 40)}" [terms: ${terms.join(",")}${shortTerms.length ? " +short:" + shortTerms.join(",") : ""}] -> ${articles.length} article(s) + ${slack.length} slack fact(s)`);
