...
export async function storeArticle(articleId, factText, sourceRef, confidence = 0.75) {
  ...
  try {
    ...
    // New article: insert. (article_id unique guards against races.)
    await pool.query(
      `INSERT INTO knowledge (fact, source_type, source_ref, confidence, content_hash, article_id)
       VALUES ($1, 'article', $2, $3, $4, $5)
       ON CONFLICT (article_id) DO UPDATE SET
         fact = EXCLUDED.fact, content_hash = EXCLUDED.content_hash,
         source_ref = EXCLUDED.source_ref, embedding = NULL, updated_at = now()
       WHERE knowledge.content_hash <> EXCLUDED.content_hash;`,
